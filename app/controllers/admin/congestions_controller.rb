class Admin::CongestionsController < ApplicationController
  ...
  def index
    ...
    @receptions = Reception.where(examination_status: '発行済').page(params[:page]).order(created_at: :desc)
    @reservations = Reservation.where(examination_status: '受診前').page(params[:page]).order(created_at: :desc)
  end
